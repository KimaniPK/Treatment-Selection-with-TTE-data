...
###########################################################################################################
###########################################################################################################
############ THIS IS THE R CODE FOR PERFORMING THE COMPUTATIONS IN SECTION 5 IN THE MAIN PAPER ############
###########################################################################################################
###########################################################################################################
...
## Creating indicators for Treatment 1 and Treatment 2 and fitting Cox's model ##

y.onepartition <- y1
y.onepartition$Group1 <- 1 * (y.onepartition$Arm == 1) # Indicator for Treatment 1
y.onepartition$Group2 <- 1 * (y.onepartition$Arm == 2) # Indicator for Treatment 2

Cox <- coxph(SurvTime ~ Group1 + Group2, data = y.onepartition) # Cox's proportional hazard model to stage 1 data
InfoMatrix <- Cox$var # Variance-covariance matrix of stage 1 log hazard ratios
Cox <- summary(Cox)


## Stage 1 Treatment 1 estimates ##

T1.stage1 <- c(Cox$coefficients[1,1], InfoMatrix[1,1]) ## Treatment 1 log hazard ratio estimate $\hat{\theta}_{1,1}$ and its variance
...
## Stage 1 Treatment 2 estimates ##

T2.stage1 <- c(Cox$coefficients[2,1], InfoMatrix[2,2]) ## Treatment 2 log hazard ratio estimate $\hat{\theta}_{1,2}$ and its variance
...
## Stage 1 covariance between treatment 1 and treatment 2 log hazard ratios ##

covT1T2 <- 	InfoMatrix[1,2] # This is $q_{12}$
...
y.onepartition <- y1
y.onepartition$DropArm2Stage2 <- 1 * ((y.onepartition$Stage == 1) * (y.onepartition$Arm == 2)) ## Indicator of 
y.onepartition <- y.onepartition[y.onepartition$DropArm2Stage2 == 0,]
y.onepartition$Group1 <- 1 * (y.onepartition$Arm == 1) # Indicator for Treatment 1
y.onepartition$Group2 <- 1 * (y.onepartition$Arm == 2) # Indicator for Treatment 2

Cox <- coxph(SurvTime ~ Group1 + Group2, data = y.onepartition)
InfoMatrix <- Cox$var
Cox <- summary(Cox)

T1.naive <- c(Cox$coefficients[1,1], InfoMatrix[1,1]) # Vector of Treatment 1 log hazard ratio naive estimate $\hat{\theta}_1$ and its variance
...
y.onepartition <- y1
y.onepartition$DropArm1Stage2 <- 1 * ((y.onepartition$Stage == 1) * (y.onepartition$Arm == 1)) ## Indicator of 
y.onepartition <- y.onepartition[y.onepartition$DropArm1Stage2 == 0,]
y.onepartition$Group1 <- 1 * (y.onepartition$Arm == 1) # Indicator for Treatment 1
y.onepartition$Group2 <- 1 * (y.onepartition$Arm == 2) # Indicator for Treatment 2
...
T2.naive <- c(Cox$coefficients[2,1], InfoMatrix[2,2]) # Vector of Treatment 2 log hazard ratio naive estimate $\hat{\theta}_2$ and its variance
